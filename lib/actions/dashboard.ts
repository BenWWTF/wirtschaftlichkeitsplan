...
export async function getMonthlyMetrics(month: string): Promise<MonthlyMetrics | null> {
  ...
  if (plansError || !plans) {
    ...
    if (isDevelopmentMode()) {
      ...
      return generateMockMonthlyMetrics(month)
    }
    return null
  }
...
export async function getMonthlyMetricsRange(
  ...
  if (plansError || !plans) {
    console.error('Error fetching monthly plans:', plansError)
    // Return empty array in development mode (mock data will be generated by client)
    if (isDevelopmentMode()) {
      console.log('Using empty array fallback for monthly metrics range in development')
      return []
    }
    return []
  }
...
export async function getTherapyMetrics(): Promise<TherapyMetrics[]> {
  ...
  if (therapiesError || !therapies) {
    ...
    // Return mock data in development mode
    if (isDevelopmentMode()) {
      console.log('Using mock therapy metrics for development')
      return generateMockTherapyMetrics()
    }
    return []
  }
...
export async function getDashboardSummary(): Promise<DashboardSummary> {
  ...
  // Return mock data if no therapy metrics available in development mode
  if (!therapyMetrics || therapyMetrics.length === 0) {
    if (isDevelopmentMode()) {
      console.log('Using mock dashboard summary for development')
      return generateMockDashboardSummary()
    }
    // Return minimal default if not in development
    return {
      total_revenue: 0,
      total_expenses: 0,
      net_income: 0,
