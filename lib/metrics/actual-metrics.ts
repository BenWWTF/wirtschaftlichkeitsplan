...
/**
 * Calculate actual metrics from actual_sessions column
 * Represents completed business results
 *
 * @param userId - Authenticated user ID
 * @param monthYear - Month in YYYY-MM format
 * @returns Actual metrics for the specified month
 */
export async function getActualMetrics(
  ...
  try {
    ...
    const { data: plans, error: plansError } = await supabase
      .from('monthly_plans')
      .select('id, actual_sessions, therapy_type_id')
      .eq('user_id', userId)
      .eq('month', monthDate)

    if (plansError) {
      logError('getActualMetrics', 'Error fetching monthly plans', plansError, { userId, monthYear })
      throw plansError
    }
    ...
    if (therapiesError) {
      logError('getActualMetrics', 'Error fetching therapy types', therapiesError, { therapyTypeIds })
      throw therapiesError
    }

    if (!therapies || therapies.length === 0) {
      logError('getActualMetrics', 'No therapies found', new Error('Missing therapy types'), { therapyTypeIds })
      return createEmptyMetrics()
    }
    ...
    for (const plan of plans) {
      ...
      if (!therapy) {
        logError('getActualMetrics', 'Therapy not found in map', new Error('Missing therapy'), {
          therapy_id: plan.therapy_type_id
        })
        continue
      }

      // Guard: Skip if actual_sessions is null or undefined (default to 0)
      const actualSessions = plan.actual_sessions ?? 0

      // Skip therapies with zero actual sessions to keep results clean
      if (actualSessions === 0) {
        continue
      }

      const revenue = actualSessions * therapy.price_per_session
      const variableCosts = actualSessions * therapy.variable_cost_per_session
    ...
    logError('getActualMetrics', 'Unexpected error in getActualMetrics', error, { userId, monthYear })
    return createEmptyMetrics()
  }
}
