...
export function MetricExplanation({
  ...
  return (
    <Card className={cn('border-l-4 border-l-accent-500', className)}>
      <CardHeader className="pb-3 p-4 sm:p-6">
        <div className="flex items-start justify-between">
          <div className="flex items-start gap-2">
            {icon && <div className="h-4 w-4 sm:h-5 sm:w-5 text-accent-500 flex-shrink-0 mt-1">{icon}</div>}
            <div>
              <CardTitle className="text-xs sm:text-sm font-semibold text-neutral-700 dark:text-neutral-300">
                {title}
              </CardTitle>
            </div>
          </div>
      ...
      <CardContent className="space-y-3 p-4 sm:p-6 pt-0">
        ...
        <div className="bg-neutral-50 dark:bg-neutral-900 rounded-lg p-2 sm:p-3">
          <div className="text-xl sm:text-2xl font-bold text-neutral-900 dark:text-neutral-50">
            {value}
            {unit && <span className="text-xs sm:text-sm font-normal text-neutral-600 dark:text-neutral-400 ml-1">{unit}</span>}
          </div>
        </div>
          ...
          <div className="pt-2">
            <Link href={actionHref} className="block">
              <Button
                variant="outline"
                size="sm"
                className="w-full min-h-[44px] text-accent-600 dark:text-accent-400 border-accent-200 dark:border-accent-800 hover:bg-accent-50 dark:hover:bg-accent-900/20"
              >
                {actionLabel}
              </Button>
            </Link>
