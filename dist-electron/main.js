...
function createWindow() {
  mainWindow = new BrowserWindow({
    ...
    minHeight: 600,
    webPreferences: {
      preload: path__default.join(__dirname$1, "preload.cjs"),
...
app.on("ready", () => {
  const dbPath = path__default.join(app.getPath("userData"), "wirtschaftlichkeitsplan.db");
...
ipcMain.handle("sync:set-config", async (event, config2) => {
